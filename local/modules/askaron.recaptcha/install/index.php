...
use askaronReCaptcha\Module as M;

Loc::loadMessages(__FILE__);

class askaron_recaptcha extends CModule
{
	var $MODULE_ID = 'askaron.recaptcha'; // без этого не проходит валидация при загрузке архива когда добавляем в маркетплейс
	var $MODULE_VERSION;
    ...
    public function __construct()
    {
        ...
        $this->PARTNER_NAME = Loc::getMessage('askaron_RECAPTCHA_PARTNER_NAME'); // без этого не проходит валидация при загрузке архива когда добавляем в маркетплейс
        $this->PARTNER_URI = Loc::getMessage('askaron_RECAPTCHA_PARTNER_URI'); // без этого не проходит валидация при загрузке архива когда добавляем в маркетплейс
    }
    ...
    public function InstallEvents()
    {
        ...
        // проверка на спам
        $EventManager->registerEventHandler('main', 'OnPageStart', $this->MODULE_ID, 'askaronReCaptcha\BitrixCaptcha', 'initCheckSpam');

        // инициализация js
        $EventManager->registerEventHandler('main', 'OnProlog', $this->MODULE_ID, 'askaronReCaptcha\BitrixCaptcha', 'initJS');
    }
    ...
    public function UnInstallEvents()
    {
        ...
        // проверка на спам
        $EventManager->unRegisterEventHandler('main', 'OnPageStart', $this->MODULE_ID, 'askaronReCaptcha\BitrixCaptcha', 'initCheckSpam');

        // инициализация js
        $EventManager->unRegisterEventHandler('main', 'OnProlog', $this->MODULE_ID, 'askaronReCaptcha\BitrixCaptcha', 'initJS');
    }
